  ...
  return (
    ...
    <div className='a'>
        {datas.length>0?(
            <>
       <h1> OUR TOURS</h1>
       {datas.map((data,i)=><div key={i}><div className='b'><img src={data.image} />
       <div className='c'>
               ...
               <div>
               <p>{show ?  data.info :data.info.slice(0,200)} <button className="btn btn-secondary" onClick={handletoggle}>{show?"show less":"show more"}</button>
               </p>
               <center><button className='btn btn-outline-danger m-4' onClick={()=> handledelete(i)}>NOT INTERESTED</button></center>
             </div>
            ...
            <br/>
            <button className='btn btn-primary' onClick={()=>window.location.reload()}>REFRESH</button></h1>)}
    </div>}
    </div>
  )
}
